...
long double Integrand1(long double t, long double tmax, aerofoil a) {
  long double T = t;

  T *= (t < 0.0L) * (-1.0L) + (t >= 0.0L) * (1.0L);
  long double theta = theta1(T , a);
  long double dx = 1 - f3prime(T, a) * sinl(theta) - f3(T, a) * cosl(theta) * theta1prime(T, a);
  long double dy = f1prime(T, a) + f3prime(T, a) * cosl(theta) - f3(T, a) * sinl(theta) * theta1prime(T, a);

  return sqrtl(powl(dx, 2.0L) + powl(dy, 2.0L));
};

long double Integrand2(long double t, long double tmax, aerofoil a){
  long double T = t;
  T *= (t < 0.0L) * (-1.0L) + (t >= 0.0L) * (1.0L);
 return sqrtl(powl(1 + f3prime(T, a) * sinl(theta1(T, a)) + f3(T, a) * cosl(theta1(T, a)) * theta1prime(T, a), 2.0L) + powl(f1prime(T, a) - f3prime(T, a) * cosl(theta1(T, a)) + f3(T, a) * sinl(theta1(T, a)) * theta1prime(T, a), 2.0L));
};

long double Integrand3(long double t, long double tmax, aerofoil a){
  long double T = t;
  T *= (t < 0.0L) * (-1.0L) + (t >= 0.0L) * (1.0L);
  return sqrtl(powl(1 - f3prime(T, a) * sinl(theta2(T, a)) - f3(T, a) * cosl(theta2(T, a)) * theta2prime(T, a), 2.0L) + powl(f2prime(T, a) + f3prime(T, a) * cosl(theta2(T, a)) - f3(T, a) * sinl(theta2(T, a)) * theta2prime(T, a), 2.0L));
};

long double Integrand4(long double t, long double tmax, aerofoil a){
  long double T = t;
  T *= (t < 0.0L) * (-1.0L) + (t >= 0.0L) * (1.0L);
  return sqrtl(powl(1 + f3prime(T, a) * sinl(theta2(T, a)) + f3(T, a) * cosl(theta2(T, a)) * theta2prime(T, a), 2.0L) + powl(f2prime(T, a) - f3prime(T, a) * cosl(theta2(T, a)) + f3(T, a) * sinl(theta2(T, a)) * theta2prime(T, a), 2.0L));
};

long double arclengthGauss(long double t1, long double t2, long double t3,
                      aerofoil a, Gauss G[100]) {

  long double result = 0.0L;
  for (int i = 0; i < 100; i++) {
    result += ((t2 - t1) / 2.0L) * G[i].weight *
              Integrand1((G[i].node + 1) * (t2 - t1) / 2 + t1, t3, a);
  };
  return result;
};

long double arclengthTanh(long double t1, long double t2, long double tmax,
                          aerofoil a, int section) {
  long double h = 0.04;
  int bounds = 79;
  long double result = 0.0L;
  switch (section){
  ...
  for (int k = -bounds; k <= bounds; k++) {
    result += Integrand1(0.5 * (t2 - t1) * tanhl(0.5 * M_PI * sinhl(k * h)) + 0.5 * (t1 + t2), tmax, a) * coshl(k * h) / powl(cosh(0.5 * M_PI * sinhl(k * h)), 2.0L);
  };
  break;
  case 2:
  for (int k = -bounds; k <= bounds; k++) {
    result += Integrand2(0.5 * (t2 - t1) * tanhl(0.5 * M_PI * sinhl(k * h)) + 0.5 * (t1 + t2), tmax, a) * coshl(k * h) / powl(cosh(0.5 * M_PI * sinhl(k * h)), 2.0L);
  };
  break;
  case 3:
  for (int k = -bounds; k <= bounds; k++) {
    result += Integrand3(0.5 * (t2 - t1) * tanhl(0.5 * M_PI * sinhl(k * h)) + 0.5 * (t1 + t2), tmax, a) * coshl(k * h) / powl(cosh(0.5 * M_PI * sinhl(k * h)), 2.0L);
  };
  break;
  case 4:
  for (int k = -bounds; k <= bounds; k++) {
    result += Integrand4(0.5 * (t2 - t1) * tanhl(0.5 * M_PI * sinhl(k * h)) + 0.5 * (t1 + t2), tmax, a) * coshl(k * h) / powl(cosh(0.5 * M_PI * sinhl(k * h)), 2.0L);
  };
  break;
  default:
  printf("Invalid section. section = [1, 4]\n");
  exit(1);
  };
  result *= (h * 0.25L * (t2 - t1) * M_PI);
  return result;
}


void arcadjust(Tarray *array, aerofoil a, int *ncrit, int *defaulter, MeshFineness *Mesh1,
  ...
  int section = 1;
  d1 = arclengthGauss(t0, array->T[1], t3, a, G);
  d2 = arclengthTanh(0.0, t0, t3, a, section);
  int fefOG = (((int)(d2 / d1))) * (Mesh1->IsSym == 0) + 1 * (Mesh1->IsSym != 0);
  ...
  if (fefOG == 1 && (Mesh1->IsSym == 0) ) {
    array->T = realloc(array->T, (array->size + fefOG) * sizeof(long double));
    memmove(array->T + fefOG, array->T, (array->size) * sizeof(long double));
    array->T[0] = 0;
    array->size += 1;
	printf("0 point printed\n");
  };
   fefOG *= (Mesh1->IsSym == 0);
  *ncrit = fefOG;
   printf("ncrit = %d\n", *ncrit);
...
int ReadMeshParam(FILE *inputfile, MeshFineness *Mesh1, aerofoil *aero, MeshParameters Mesh, BoundaryConditions* BB3) {
  int j = 0;
  ...
  if (Mesh == ONE_PARAMETER){
	  j += 5;
	  goto MeshParameterReading_Done;
  }
...
MeshParameterReading_Done:
  PrintData(Mesh1, aero);
  return j;
}

void infoprint(MeshParameters Mesh) {
  printf("xL = ...\n");
  printf("xR = ...\n");
  printf("yB = ...\n");
  printf("yU = ...\n");
  ...
  printf("c = ...\n");
  printf("mp = ...\n");
  if (Mesh == ONE_PARAMETER) return;
  printf("r_num = ...\n");
  printf("r_dem = ...\n");
  printf("gp = ...\n");
  printf("cpar = ...\n");
  printf("tpar = ...\n");
  if (Mesh == ALL_PARAMETERS) return;
  printf("IsSym = ...\n");
  printf("Ux = ...\n");
  printf("Uy = ...\n");
}
...
int main(int argc, char *argv[]) {
  ...
  if (FileProvided == 1){
  op59 = ReadMeshParam(inputfile, &Mesh1, &aero, FileType, &BB3);
  };
  xL = Mesh1.xL;
  xR = Mesh1.xR;
  yU = Mesh1.yU;
...
t2fix:
  ...
  if(FileType != ONE_PARAMETER) goto All_Param;

  long double d1 = 0;
  long double d2 = 0;

  d1 = arclengthTanh(0.0, p[1], p[3], aero, 1);
  d2 = arclengthTanh(0.0, p[2], p[3], aero, 2);
  Mesh1.r_dem = Mesh1.mp;
  Mesh1.IsSym = 2*(fabsl(aero.m) < 1e-3);
  Mesh1.r_num = (int) ((d2 / d1) * ((long double) (mp)) + 0.5L);
  long double d1_previous = d1;
  d1 += arclengthTanh(p[1], aero.p * aero.c, p[3], aero, 1);
  d1 += arclengthTanh(aero.p * aero.c, p[3] - 1e-10, p[3], aero, 3);
  d2 += arclengthTanh(p[2], aero.p * aero.c, p[3], aero, 2);
  d2 += arclengthTanh(aero.p * aero.c, p[3] - 1e-10, p[3], aero, 4);
  Mesh1.gp = 4 * mp;
  Mesh1.Cpar = 0.095;
  Mesh1.Tpar = 6;
  PrintData(&Mesh1, &aero); 
  ...
  arcadjust(array, aero, &ncrit, &defaulter, &Mesh1, t3, G);
