...
long double f3(long double t, aerofoil a) {
  return (5 * a.T * (a.a[0] * sqrt(t / a.c) + a.a[1] * (t / a.c) + a.a[2] * pow((t / a.c), 2.0) + a.a[3] * pow((t / a.c), 3.0) + a.a[4] * pow((t / a.c), 4.0)));
}
...
long double f3prime(long double t, aerofoil a) {
  return (5 * a.T * (a.a[0] * (1 / (2 * sqrt(a.c * t))) + a.a[1] / a.c + 2 * a.a[2] * t / (a.c * a.c) + 3 * a.a[3] * t * t / (pow(a.c, 3.0)) + 4 * a.a[4] * pow(t, 3.0) / (pow(a.c, 4.0))));
}

long double theta1prime(long double t, aerofoil a) {
  return (-2 * a.kk / ((a.c * a.c) * (1 + pow((2 * a.kk * (a.p / a.c - t / (a.c * a.c))), 2.0))));
}

long double theta2prime(long double t, aerofoil a) {
  return (-2 * a.jj / ((a.c * a.c) * (1 + pow((2 * a.jj * (a.p / a.c - t / (a.c * a.c))), 2.0))));
}

long double dydx1(long double t, aerofoil a) {
  return ((f1prime(t, a) + f3prime(t, a) * cos(theta1(t, a)) - theta1prime(t, a) * f3(t, a) * sin(theta1(t, a))) / (1.0 - f3prime(t, a) * sin(theta1(t, a)) - theta1prime(t, a) * f3(t, a) * cos(theta1(t, a))));
}

long double dydx2(long double t, aerofoil a) {
  return (f1prime(t, a) - f3prime(t, a) * cos(theta1(t, a)) + theta1prime(t, a) * f3(t, a) * sin(theta1(t, a))) / (1 + f3prime(t, a) * sin(theta1(t, a)) + theta1prime(t, a) * f3(t, a) * cos(theta1(t, a)));
}

long double dydx3(long double t, aerofoil a) {
  return (f2prime(t, a) + f3prime(t, a) * cos(theta2(t, a)) - theta2prime(t, a) * f3(t, a) * sin(theta2(t, a))) / (1 - f3prime(t, a) * sin(theta2(t, a)) - theta2prime(t, a) * f3(t, a) * cos(theta2(t, a)));
}
...
long double Zeroroot(long double testval, aerofoil aero) {
  return (1 - f3prime(testval, aero) * sin(theta1(testval, aero)) - f3(testval, aero) * theta1prime(testval, aero) * cos(theta1(testval, aero))) / (f1prime(testval, aero) + f3prime(testval, aero) * cos(theta1(testval, aero)) - theta1prime(testval, aero) * f3(testval, aero) * sin(theta1(testval, aero)));
};

long double Firstroot(long double t, aerofoil a, long double xL, long double yU) {
  // f1(t) + f3(t) * cos (theta1(t) );
  // t - f3(t) * sin (theta1(t) );

  return ((yU - (f1(t, a) + f3(t, a) * cos(theta1(t, a)))) / (xL - t + f3(t, a) * sin(theta1(t, a)))) + Zeroroot(t, a);
};
long double Secondroot(long double t, aerofoil a, long double xL, long double yB) {

  return ((yB - (f1(t, a) - f3(t, a) * cos(theta1(t, a)))) / (xL - t - f3(t, a) * sin(theta1(t, a)))) + (1 + f3prime(t, a) * sin(theta1(t, a)) + f3(t, a) * theta1prime(t, a) * cos(theta1(t, a))) / (f1prime(t, a) - f3prime(t, a) * cos(theta1(t, a)) + theta1prime(t, a) * f3(t, a) * sin(theta1(t, a)));
};
...
void GfaceFILL(FILE *faces[3], int o4, int o3, int o2, int o1, int *flabel, int bcode, cell *oker, int faceN, int *clabel) {
  faceN++;
...
/*
  int nFaces[8] = {
      2 * ntip,                                  // Aerofoil 		nFaces[0]
      mp + (mp * r_num) / r_dem + 2 * ncrit,     // Inlet    		nFaces[1]
      2 * (j + nodes - 1),                       // Outlet   		nFaces[2]
      2 * (2 * ntip + 2 * bc) * (j + nodes - 1), // FrontAndBack    nFaces[3]
      ntip - mp - ncrit,                         // Top1 			nFaces[4]
      ntip - ncrit - (mp * r_num) / r_dem,       // bottom1			nFaces[5]
      bc,                                        // top2            nFaces[6]
      bc                                         // bottom2         nFaces[7]
  };


  int startFace[8] = {nFaces[3],                                                // Aerofoil
                      nFaces[3] + 2 * ntip,                                     // Inlet
                      nFaces[3] + 2 * ntip + nFaces[1],                         // Outlet
                      0,                                                        // FrontAndBack
                      nFaces[3] + 2 * ntip + nFaces[1] + nFaces[2],             // Top1
                      nFaces[3] + 2 * ntip + nFaces[1] + nFaces[2] + nFaces[4], // bottom1
                      nFaces[3] + 2 * ntip + nFaces[1] + nFaces[2] + nFaces[4] + nFaces[5], //Top2
                                          nFaces[3] + 2 * ntip + nFaces[1] + nFaces[2] + nFaces[4] + nFaces[5] + nFaces[6], //Bottom2
...
void ownFILL(FILE *owner[2], FILE *neighbour[2], int clabelo, int clabeln, facecode *bc) {

  if (*bc != AEROFOIL && *bc != IN && *bc != OUT && *bc != INLET && *bc != OUTLET && *bc != TOP && *bc != BOTTOM && *bc != TOP2 && *bc != BOTTOM2) {
...
void faceFILL(FILE *faces[3], int o1, int o2, int o3, int o4, int *flabel, int bcode, cell *oker, int faceN, int *clabel) {
  oker->faces[faceN] = *flabel;
  oker->label = *clabel;
...
void onlyFILL(FILE *faces[3], int o1, int o2, int o3, int o4, int *flabel, int bcode) {
  fprintf(faces[0], "4(%d %d %d %d)\n", o1, o2, o3, o4);
  // fprintf(faces[0], "( %d %8d %8d %8d )\n", o1, o2, o3, o4);
  fprintf(faces[1], "( %d %8d %8d %8d )\t%8d\t%8d\n", o1, o2, o3, o4, bcode, *flabel);
  // fprintf(faces[2], "%d\t%d\t%d\t%d\t%8d\t%8d\n", o1, o2, o3, o4, bcode, *flabel);
...
void WriteToPoint(const void *restrict x, const void *restrict y, const void *restrict z0, const void *restrict label, FILE *pointoutput) {
...
void linestack(FILE *pointoutput[4], foilpoint *foil, foilpoint *zoinks, aerofoil a, long double cpar, Tarray array, int ncrit, long double xL, int mp, int r_num, int r_dem, long double xR, long double yU, long double yB, int backtrace, trace *trace1, long double slopeshift, FILE *faces[3], FILE *cells[3], FILE *owner[2], FILE *neighbour[2], FILE *boundar[2], OPTIONB opt1) {
  ...
  printf("\n t val of bottom left corner = %.15Lf", foil->associatedT[ncrit + (int)(2 * mp / 3)]);
  printf("\n last element of foil count is %.15Lf", foil->associatedT[foil->count - ncrit - (int)(2 * mp / 3)]);
  ...
  while (x2 > (3.0 / 5.0) * xL) {
    ...
    while (n < foil->count) {
      if (foil->section[n] == 1) {
        ...
        fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[n], foil->y[n], 0.0L, 2 * label1);
        fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[n], foil->y[n], delz, 2 * label1 + 1);

        WriteToPoint(&foil->x[n], &foil->y[n], &(long double){0.0L}, &(int){2 * label1}, pointoutput[3]);
        WriteToPoint(&foil->x[n], &foil->y[n], &(long double){delz}, &(int){2 * label1 + 1}, pointoutput[3]);

        fprintf(pointoutput[2], "(%.15Lf %.15Lf %.4Lf )\n", foil->x[n], foil->y[n], 0.0L);
        fprintf(pointoutput[2], "(%.15Lf %.15Lf %.4Lf )\n", foil->x[n], foil->y[n], delz);

        if (foil->label[n] == ncrit) {
          x2 = foil->x[n];
          printf("We're stuck here %.15Lf > %.15Lf slope = %.15Lf dx = %.15Lf \n", x2, (3.0 / 5.0) * xL, foil->slope[n], cpar * cosl(atanl(foil->slope[n])));
        };
      };
      if (foil->section[n] == 3) {
        ...
        if (foil->label[n] == ntip) {
          ...
          for (int i = 1; i <= backtrace; i++) {
            ...
            fprintf(pointoutput[0], "%.15Lf\t%.15Lf\n", trace1->x[i - 1], trace1->y);
            fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", trace1->x[i - 1], trace1->y, 0.0L, 2 * label1);
            fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", trace1->x[i - 1], trace1->y, delz, 2 * label1 + 1);

            WriteToPoint(&trace1->x[i - 1], &trace1->y, &(long double){0.0L}, &(int){2 * label1}, pointoutput[3]);
            WriteToPoint(&trace1->x[i - 1], &trace1->y, &(long double){delz}, &(int){2 * label1 + 1}, pointoutput[3]);
          ...
          fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[n], ytail, 0.0L, 2 * label1);
          fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[n], ytail, delz, 2 * label1 + 1);

          WriteToPoint(&foil->x[n], &ytail, &(long double){0.0L}, &(int){2 * label1}, pointoutput[3]);
          WriteToPoint(&foil->x[n], &ytail, &(long double){delz}, &(int){2 * label1 + 1}, pointoutput[3]);
      ...
      if (foil->section[n] == 4) {
        ...
        long double p1 = (foil->associatedT[n] - a.c * a.p) / (foil->associatedT[ntip] - a.c * a.p);
        long double p2 = 1.0 - p1;
        long double tempslope = (0.0 - foil->slope[ntip] - slopeshift) * p1 + p2 * foil->slope[n];
        if (tempslope > 0.0) {
  ...
  for (int i = 0; i < foil->count; i++) {
    ...
    if ((i > ntip) && (i < foil->count - ncrit - (int)(r_num * mp / r_dem)) && (foil->section[i] == 4)) {
      long double p1 = (foil->associatedT[i] - a.c * a.p) / (foil->associatedT[ntip] - a.c * a.p);
      long double p2 = 1.0 - p1;
      long double tempslope = (0.0 - foil->slope[ntip]) * p1 + p2 * foil->slope[i];
  ...
  for (int u = 1; u <= nodes; u++) {
    ...
    for (int v = 0; v < foil->count; v++) {
      ...
      if (0 <= v && v <= ncrit + mp) {
        ...
        fprintf(pointoutput[0], "%.15Lf\t%.15Lf\n", foil->x[v] + dx[v], foil->y[v] + dy[v]);

        fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[v] + dx[v], foil->y[v] + dy[v], 0.0L, 2 * label1);
        fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[v] + dx[v], foil->y[v] + dy[v], delz, 2 * label1 + 1);

        WriteToPoint(&(long double){foil->x[v] + dx[v]}, &(long double){foil->y[v] + dy[v]}, &(long double){0.0L}, &(int){2 * label1}, pointoutput[3]);
        WriteToPoint(&(long double){foil->x[v] + dx[v]}, &(long double){foil->y[v] + dy[v]}, &(long double){delz}, &(int){2 * label1 + 1}, pointoutput[3]);

        fprintf(pointoutput[2], "(%.15Lf %.15Lf %.4Lf )\n", foil->x[v] + dx[v], foil->y[v] + dy[v], 0.0L);
        fprintf(pointoutput[2], "(%.15Lf %.15Lf %.4Lf )\n", foil->x[v] + dx[v], foil->y[v] + dy[v], delz);
      };
      ...
      if (v == ntip) {
        ...
        tracecount++;
        for (int i = 1; i <= backtrace; i++) {
          long double deltax = (xR - (foil->x[v] + dx[v])) / ((long double)(backtrace));
        ...
        fprintf(pointoutput[0], "%.15Lf\t%.15Lf\n", foil->x[v] + dx[v + 1], ytail + dy[v + 1]);

        fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[v] + dx[v + 1], ytail + dy[v + 1], 0.0L, 2 * label1);
        fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[v] + dx[v + 1], ytail + dy[v + 1], delz, 2 * label1 + 1);

        WriteToPoint(&(long double){foil->x[v] + dx[v + 1]}, &(long double){ytail + dy[v + 1]}, &(long double){0.0L}, &(int){2 * label1}, pointoutput[3]);
        WriteToPoint(&(long double){foil->x[v] + dx[v + 1]}, &(long double){ytail + dy[v + 1]}, &(long double){delz}, &(int){2 * label1 + 1}, pointoutput[3]);
        ...
        label1++;
        for (int i = 1; i <= backtrace; i++) {
          long double deltax = (xR - (foil->x[v] + dx[v + 1])) / ((long double)(backtrace));
          trace1->x[i - 1] = ((long double)(i)) * deltax + foil->x[v] + dx[v + 1];
      ...
      if ((v > ntip) && (v < foil->count - ncrit - (int)(r_num * mp / r_dem)) && (foil->section[v] == 4)) {
        dx[v + 1] = u * ((ynet[v + 1] - foil->x[v]) / ((long double)(nodes)));
        dy[v + 1] = u * ((yB - foil->y[v]) / ((long double)(nodes)));

        fprintf(pointoutput[0], "%.15Lf\t%.15Lf\n", foil->x[v] + dx[v + 1], foil->y[v] + dy[v + 1]);

        fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[v] + dx[v + 1], foil->y[v] + dy[v + 1], 0.0L, 2 * label1);
        fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[v] + dx[v + 1], foil->y[v] + dy[v + 1], delz, 2 * label1 + 1);

        WriteToPoint(&(long double){foil->x[v] + dx[v + 1]}, &(long double){foil->y[v] + dy[v + 1]}, &(long double){0.0L}, &(int){2 * label1}, pointoutput[3]);
        WriteToPoint(&(long double){foil->x[v] + dx[v + 1]}, &(long double){foil->y[v] + dy[v + 1]}, &(long double){delz}, &(int){2 * label1 + 1}, pointoutput[3]);
  ...
  int n_InternalFaces = 2 * (2 * ntip + 2 * bc) * (j + nodes - 1) + (2 * ntip + bc) + (2 * ntip + 2 * bc) * (j + nodes - 1) + (2 * ntip + 1 + 2 * bc) * (j + nodes - 1) - (nFaces[0] + nFaces[1] + nFaces[2] + nFaces[3] + nFaces[4] + nFaces[5] + nFaces[6] + nFaces[7]);
  ...
  fprintf(faces[0], "\n\n");
  fprintf(faces[0], "%d", 2 * (2 * ntip + 2 * bc) * (j + nodes - 1) + (2 * ntip + bc) + (2 * ntip + 2 * bc) * (j + nodes - 1) + (2 * ntip + 1 + 2 * bc) * (j + nodes - 1));
  fprintf(faces[0], "\n(\n");
  fprintf(owner[0], "\n\n%d\n(\n", 2 * (2 * ntip + 2 * bc) * (j + nodes - 1) + (2 * ntip + bc) + (2 * ntip + 2 * bc) * (j + nodes - 1) + (2 * ntip + 1 + 2 * bc) * (j + nodes - 1));

  fprintf(neighbour[0], "\n\n%d\n(\n", 2 * (2 * ntip + 2 * bc) * (j + nodes - 1) + (2 * ntip + bc) + (2 * ntip + 2 * bc) * (j + nodes - 1) + (2 * ntip + 1 + 2 * bc) * (j + nodes - 1) - (nFaces[0] + nFaces[1] + nFaces[2] + nFaces[3] + nFaces[4] + nFaces[5] + nFaces[6] + nFaces[7]));
  ...
  int t2n = 2 * ntip + 2 * backtrace + 2 * ntip - 2 * ncrit - 2 * ((int)((((long double)(r_num)) / ((long double)(r_dem))) * (mp)));
  bcode = AEROFOIL;
  ...
  for (int i = t2n + loop2; i < loop1 + loop2 - 2; i += 2) {
    ...
    faceFILL(faces, i, i + 1, i + 3, i + 2, &flabel, bcode, &oker[clabel], 1, &clabel);
    I++;
  };
  i1 = loop1 + loop2 - 2;
  comp = loop1;
  oker[clabel].faces[0] = AerofoilF[I];
  ownFILL(owner, neighbour, clabel, clabel + 2 * ntip + 2 * bc, &bcode);
  faceFILL(faces, i1, i1 + 1, comp + 1, comp, &flabel, bcode, &oker[clabel], 1, &clabel);
  I++;
  ...
  for (int i = 2 * ntip + loop2; i < 2 * ntip + loop2 + 2 * bc; i += 2) {
    oker[clabel].faces[0] = Tippy[I];
    ownFILL(owner, neighbour, clabel, clabel + 2 * ntip + 2 * bc, &bcode);
    faceFILL(faces, i, i + 2, i + 3, i + 1, &flabel, bcode, &oker[clabel], 1, &clabel);
    I++;
  };
  ...
  while (k < j + nodes - 1) {
    ...
    for (int i = loop1 + (k - 1) * loop2; i < loop1 + (k - 1) * loop2 + 2 * ntip; i += 2) {
      oker[clabel].faces[0] = flabel - 2 * ntip - 2 * bc;
      ownFILL(owner, neighbour, clabel, clabel + 2 * ntip + 2 * bc, &bcode);
      faceFILL(faces, i, i + 1, i + 3, i + 2, &flabel, bcode, &oker[clabel], 1, &clabel);
    }
    ...
    for (int i = loop1 + (k - 1) * loop2 + 2 * ntip; i < loop1 + (k - 1) * loop2 + 2 * ntip + 2 * bc; i += 2) {
      oker[clabel].faces[0] = flabel - 2 * ntip - 2 * bc;
      ownFILL(owner, neighbour, clabel, clabel + 2 * ntip + 2 * bc, &bcode);
      faceFILL(faces, i, i + 1, i + 3, i + 2, &flabel, bcode, &oker[clabel], 1, &clabel);
    };

    for (int i = 2 * ntip + k * loop2; i < 2 * ntip + k * loop2 + 2 * bc; i += 2) {
      oker[clabel].faces[0] = flabel - 2 * ntip - 2 * bc;
      ownFILL(owner, neighbour, clabel, clabel + 2 * ntip + 2 * bc, &bcode);
      faceFILL(faces, i, i + 2, i + 3, i + 1, &flabel, bcode, &oker[clabel], 1, &clabel);
    };
  ...
  faceFILL(faces, i1, i1 + 1, comp + 1, comp, &flabel, bcode, &oker[clabel], 3, &clabel);

  for (int i = t2n + 2; i < loop1; i += 2) {
    ownFILL(owner, neighbour, clabel - 1, clabel, &bcode);
    oker[clabel].faces[2] = flabel + 1;
    faceFILL(faces, i, i + loop2, i + loop2 + 1, i + 1, &flabel, bcode, &oker[clabel], 3, &clabel);
  };

  for (int i = 0; i < 2 * ntip; i += 2) {
    ownFILL(owner, neighbour, clabel - 1, clabel, &bcode);
    oker[clabel].faces[2] = flabel + 1;
    faceFILL(faces, i, i + loop1, i + loop1 + 1, i + 1, &flabel, bcode, &oker[clabel], 3, &clabel);
  };
  dlabel = flabel;
  ownFILL(owner, neighbour, clabel - 1, clabel - 1 + nFaces[Bottom1] + 1, &bcode);
  onlyFILL(faces, 2 * ntip, 2 * ntip + loop1, 2 * ntip + loop1 + 1, 2 * ntip + 1, &flabel, bcode);
  flabel++;

  oker[clabel].faces[2] = flabel + 1;
  ownFILL(owner, neighbour, clabel, clabel + nFaces[Bottom1] + bc, &bcode);
  faceFILL(faces, 2 * ntip, 2 * ntip + 1, 2 * ntip + 1 + loop2, 2 * ntip + loop2, &flabel, bcode, &oker[clabel], 3, &clabel);
  ...
  ownFILL(owner, neighbour, clabel - 1, clabel, &bcode);
  i1 = t2n - 2;
  faceFILL(faces, i1, i1 + loop2, i1 + loop2 + 1, i1 + 1, &flabel, bcode, &oker[clabel], 3, &clabel);

  i1 = 2 * ntip + 2;
  oker[clabel].faces[3] = dlabel;
  ownFILL(owner, neighbour, clabel, clabel + 1, &bcode);
  faceFILL(faces, i1, i1 + loop1, i1 + loop1 + 1, i1 + 1, &flabel, bcode, &oker[clabel], 2, &clabel);

  for (int i = 2 * ntip + 4; i < 2 * ntip + 2 * bc; i += 2) {
    oker[clabel].faces[3] = flabel - 1;
    ownFILL(owner, neighbour, clabel, clabel + 1, &bcode);
    faceFILL(faces, i, i + loop1, i + loop1 + 1, i + 1, &flabel, bcode, &oker[clabel], 2, &clabel);
  };
  oker[clabel].faces[3] = flabel - 1;
  ...
  ownFILL(owner, neighbour, clabel, clabel + 1, &bcode);
  faceFILL(faces, i1, i1 + 1, i1 + 1 + loop2, i1 + loop2, &flabel, bcode, &oker[clabel], 2, &clabel);

  for (int i = 2 * ntip + 4; i < 2 * ntip + 2 * bc; i += 2) {
    oker[clabel].faces[3] = flabel - 1;
    ownFILL(owner, neighbour, clabel, clabel + 1, &bcode);
    faceFILL(faces, i, i + 1, i + 1 + loop2, i + loop2, &flabel, bcode, &oker[clabel], 2, &clabel);
  };
  oker[clabel].faces[3] = flabel - 1;
  ...
  while (k < j + nodes - 1) {
    ...
    for (int i = 2 * ntip + k * loop2 + 2 * bc + 2; i < t2n + k * loop2 - 2; i += 2) {
      ownFILL(owner, neighbour, clabel - 1, clabel, &bcode);
      oker[clabel].faces[2] = flabel + 1;
      faceFILL(faces, i, i + loop2, i + loop2 + 1, i + 1, &flabel, bcode, &oker[clabel], 3, &clabel);
    };
    ...
    ownFILL(owner, neighbour, clabel - 1, clabel, &bcode);
    oker[clabel].faces[2] = I;
    faceFILL(faces, i1, comp, comp + 1, i1 + 1, &flabel, bcode, &oker[clabel], 3, &clabel);

    i1 = 2 * ntip + 2 + loop1 + (k - 1) * loop2;
    comp = i1 + loop2;
    ownFILL(owner, neighbour, clabel, clabel + 1, &bcode);
    oker[clabel].faces[3] = dlabel;
    faceFILL(faces, i1, comp, comp + 1, i1 + 1, &flabel, bcode, &oker[clabel], 2, &clabel);

    for (int i = 2 * ntip + loop1 + (k - 1) * loop2 + 4; i < 2 * ntip + loop1 + (k - 1) * loop2 + 2 * bc; i += 2) {

      ownFILL(owner, neighbour, clabel, clabel + 1, &bcode);
      oker[clabel].faces[3] = flabel - 1;
      faceFILL(faces, i, i + loop2, i + loop2 + 1, i + 1, &flabel, bcode, &oker[clabel], 2, &clabel);
    };
    oker[clabel].faces[3] = flabel - 1;
    ...
    i1 = 2 * ntip + 2 + k * loop2;
    comp = i1 + loop2;
    faceFILL(faces, i1, i1 + 1, comp + 1, comp, &flabel, bcode, &oker[clabel], 2, &clabel);

    for (int i = 2 * ntip + 4 + k * loop2; i < 2 * ntip + k * loop2 + 2 * bc; i += 2) {
  ...
  for (int i = t2n; i < loop1 - 2; i++) {
    ...
    faceFILL(faces, i, i + loop2, i + loop2 + 2, i + 2, &flabel, bcode, &oker[clabel], 4, &clabel);
  ...
  comp = 0;
  ownFILL(owner, neighbour, clabel, -1, &bcode);
  faceFILL(faces, i1, i1 + loop2, loop1, 0, &flabel, bcode, &oker[clabel], 4, &clabel);

  for (int i = 0; i < 2 * ntip; i++) {
    ownFILL(owner, neighbour, clabel, -1, &bcode);
    faceFILL(faces, i, i + loop1, i + loop1 + 2, i + 2, &flabel, bcode, &oker[clabel], 4, &clabel);
  ...
  comp = 2 * ntip + 2 * bc + 2;
  ownFILL(owner, neighbour, clabel, -1, &bcode);
  faceFILL(faces, i1, i1 + loop2, comp + loop2, comp, &flabel, bcode, &oker[clabel], 4, &clabel);
  ...
  for (int i = 2 * ntip + 2; i <= 2 * ntip + 2 * bc; i++) {
    ...
    ownFILL(owner, neighbour, clabel, -1, &bcode);
    faceFILL(faces, i, i - 2, i - 2 + loop1, i + loop1, &flabel, bcode, &oker[clabel], 4, &clabel);
    i++;
  };
  ...
  while (k < j + nodes - 1) {
    ...
    for (int i = loop1 + (k - 1) * loop2; i < loop1 + (k - 1) * loop2 + 2 * ntip; i++) {
    ...
    for (int i = 2 * ntip + 2 + loop1 + (k - 1) * loop2; i <= 2 * ntip + loop1 + (k - 1) * loop2 + 2 * bc; i++) {

      ownFILL(owner, neighbour, clabel, -1, &bcode);
      faceFILL(faces, i, i - 2, i - 2 + loop2, i + loop2, &flabel, bcode, &oker[clabel], 4, &clabel);
      i++;
    };

    for (int i = 2 * ntip + 4 + loop1 + (k - 1) * loop2 + 2 * bc; i <= 2 * ntip + 2 + loop1 + (k - 1) * loop2 + 4 * bc; i++) {
  ...
  for (int i = t2n; i < loop1 - 2; i++) {
    ownFILL(owner, neighbour, clabel, -1, &bcode);
    faceFILL(faces, i + 3, i + loop2 + 3, i + loop2 + 1, i + 1, &flabel, bcode, &oker[clabel], 5, &clabel);
    i++;
  };
  i1 = loop1 - 2;
  comp = 0;
  ...
  ownFILL(owner, neighbour, clabel, -1, &bcode);
  GfaceFILL(faces, i1, i1 + loop2, loop1, 0, &flabel, bcode, &oker[clabel], 4, &clabel);

  for (int i = 0; i < 2 * ntip; i++) {
    ownFILL(owner, neighbour, clabel, -1, &bcode);
    GfaceFILL(faces, i, i + loop1, i + loop1 + 2, i + 2, &flabel, bcode, &oker[clabel], 4, &clabel);
    i++;
  };
  i1 = 2 * ntip;
  comp = 2 * ntip + 2 * bc + 2;
  ...
  ownFILL(owner, neighbour, clabel, -1, &bcode);
  GfaceFILL(faces, i1, i1 + loop2, comp + loop2, comp, &flabel, bcode, &oker[clabel], 4, &clabel);

  for (int i = 2 * ntip + 2 * bc + 2; i < t2n; i++) {
    ownFILL(owner, neighbour, clabel, -1, &bcode);
    GfaceFILL(faces, i, i + loop2, i + loop2 + 2, i + 2, &flabel, bcode, &oker[clabel], 4, &clabel);
    i++;
  };
  ...
  for (int i = 2 * ntip + 2; i <= 2 * ntip + 2 * bc; i++) {
    ...
    ownFILL(owner, neighbour, clabel, -1, &bcode);
    GfaceFILL(faces, i, i - 2, i - 2 + loop1, i + loop1, &flabel, bcode, &oker[clabel], 4, &clabel);
    i++;
  };
  ...
  while (k < j + nodes - 1) {
    ...
    for (int i = 2 * ntip + 2 + loop1 + (k - 1) * loop2; i <= 2 * ntip + loop1 + (k - 1) * loop2 + 2 * bc; i++) {
      ...
      ownFILL(owner, neighbour, clabel, -1, &bcode);
      GfaceFILL(faces, i, i - 2, i - 2 + loop2, i + loop2, &flabel, bcode, &oker[clabel], 4, &clabel);
      i++;
    };
  ...
  clabel = 0;
  BoundaryPatch p064;
  // Printing order is Front, Back, Aerofoil, Inlet, Outlet, Top1, Bottom1, Top2, Bottom2
  ...
  strcpy(name11[Aerofoil], "Aerofoil"), strcpy(name11[Inlet], "Inlet"), strcpy(name11[Outlet], "Outlet"), strcpy(name11[FrontAndBack], "FrontAndBack"), strcpy(name11[Top1], "Top1"), strcpy(name11[Bottom1], "Bottom1"), strcpy(name11[Top2], "Top2"), strcpy(name11[Bottom2], "Bottom2");

  strcpy(type11[Aerofoil], "wall"), strcpy(type11[Inlet], "patch"), strcpy(type11[Outlet], "patch"), strcpy(type11[FrontAndBack], "empty"), strcpy(type11[Top1], "patch"), strcpy(type11[Bottom1], "patch"), strcpy(type11[Top2], "patch"), strcpy(type11[Bottom2], "patch");

  strcpy(physicalType11[Aerofoil], "wall"), strcpy(physicalType11[Inlet], "inlet"), strcpy(physicalType11[Outlet], "outlet"), strcpy(physicalType11[FrontAndBack], "empty"), strcpy(physicalType11[Top1], "patch"), strcpy(physicalType11[Bottom1], "patch"), strcpy(physicalType11[Top2], "patch"), strcpy(physicalType11[Bottom2], "patch");
  ...
  for (int i = loop1 + (k - 1) * loop2; i < loop1 + (k - 1) * loop2 + 2 * ncrit + 2 * mp; i += 2) {
    ownFILL(owner, neighbour, clabel, -1, &bcode);
    onlyFILL(faces, i, i + 1, i + 3, i + 2, &flabel, bcode);
    InletF[flabel - startFace[Inlet]] = flabel;
    flabel++;
  ...
  for (int i = 2 * ntip + 2 * bc + k * loop2; i > 2 * ntip + 2 * bc; i -= loop2) {
  ...
  clabel = (j + nodes - 2) * (2 * ntip + 2 * bc) + mp + (mp * r_num) / r_dem + 2 * ncrit;

  k = j + nodes - 2;

  for (int i = loop1 + k * loop2 + 2 * (mp + ncrit); i < loop1 + k * loop2 + 2 * ntip; i += 2) {
  ...
  for (int i = loop1 + k * loop2 + 2 * ntip; i < loop1 + k * loop2 + 2 * ntip + 2 * bc; i += 2) {
  ...
  for (int J = 0; J < JMax; J++) {
    ...
    fprintf(boundar[1], "%s\t%d\t%d\n", GetBoundaryName(J), nFaces[J], startFace[J]);
  };
  fprintf(boundar[0], ")\n\n// ************************************************************************* //");
  printf("Boundary printed correctly\n");

  for (int I5 = 0; I5 < sik; I5++) {
    fprintf(cells[0], "6(%d %d %d %d %d %d)\n", oker[I5].faces[0], oker[I5].faces[1], oker[I5].faces[2], oker[I5].faces[3], oker[I5].faces[4], oker[I5].faces[5]);

    fwrite(oker[I5].faces, sizeof(int), 6, cells[1]);
    fwrite(&oker[I5].label, sizeof(int), 1, cells[1]);
    // fprintf(cells[1], " %d %d %d %d %d %d\t%d\n", oker[I].faces[0], oker[I].faces[1], oker[I].faces[2], oker[I].faces[3], oker[I].faces[4], oker[I].faces[5], oker[I].label);

    fprintf(cells[2], "( %d %d %d %d %d %d )\t%d\n", oker[I5].faces[0], oker[I5].faces[1], oker[I5].faces[2], oker[I5].faces[3], oker[I5].faces[4], oker[I5].faces[5], oker[I5].label);
  };
  ...
  printf("\n Total poitns = loop1 + (j + nodes - 1) loop2 = %d", loop1 + (j + nodes - 1) * loop2);
  printf("\n n value of t2 %d", 2 * ntip + 2 * backtrace + 2 * ntip - 2 * ncrit - 2 * ((int)((((long double)(r_num)) / ((long double)(r_dem))) * (mp))));

  printf("\n Total faces calculated: %d, Total faces: %d", 2 * (2 * ntip + 2 * bc) * (j + nodes - 1) + (2 * ntip + bc) + (2 * ntip + 2 * bc) * (j + nodes - 1) + (2 * ntip + 1 + 2 * bc) * (j + nodes - 1), flabel);

  printf("\n n value of t2, saved as t2n %d", t2n);
  printf("\n n value of t1 %d", 2 * ncrit + 2 * mp);
  printf("\n t(x,y) val of corner t1 = %.15Lf\t%.15Lf\n", foil->associatedT[ncrit + mp] - f3(foil->associatedT[ncrit + mp], a) * sinl(theta1(foil->associatedT[ncrit + mp], a)), f1(foil->associatedT[ncrit + mp], a) + f3(foil->associatedT[ncrit + mp], a) * cosl(theta1(foil->associatedT[ncrit + mp], a)));
  long double tmpt = foil->associatedT[ncrit + (int)(2 * mp / 3)];
  printf("\n t(x,y) val of bottom left corner t2 = %.15Lf\t%.15Lf\n", tmpt + f3(tmpt, a) * sinl(theta1(tmpt, a)), f1(tmpt, a) - f3(tmpt, a) * cosl(theta1(tmpt, a)));
  printf("\nTotal cell count is allegedly %d\n", sik);
  printf("\np1 = %.7LF, p2 = %.7Lf", (foil->associatedT[ntip - 1] - a.c * a.p) / (foil->associatedT[ntip] - a.c * a.p), 1 - (foil->associatedT[ntip - 1] - a.c * a.p) / (foil->associatedT[ntip] - a.c * a.p));
...
void foilcompute(FILE *pointoutput[3], foilpoint *foil, aerofoil a, Tarray *array, double tmax, int backtrace, trace *trace1, long double xR) {
  ...
  foil->associatedT = (long double *)malloc((foil->count) * sizeof(long double));
  ...
  while (*(array->T + n) < a.p * a.c) {
    ...
    fprintf(pointoutput[0], "%.15Lf\t%.15Lf\n", foil->x[n], foil->y[n]);
    //   printf("t(%d) = %.15Lf\t%.15Lf\t%.15Lf\n", foil->label[n], foil->associatedT[n], foil->x[n], foil->y[n]);
    fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[n], foil->y[n], 0.0L, 2 * foil->label[n]);

    fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[n], foil->y[n], delz, 2 * foil->label[n] + 1);

    WriteToPoint(&foil->x[n], &foil->y[n], &(long double){0.0L}, &(int){2 * foil->label[n]}, pointoutput[3]);
    WriteToPoint(&foil->x[n], &foil->y[n], &(long double){delz}, &(int){2 * foil->label[n] + 1}, pointoutput[3]);
  ...
  while (n < array->size) {
    ...
    if (n == array->size - 1) {
      ...
      for (int i = 1; i <= backtrace; i++) {
        ...
        fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", trace1->x[i - 1], trace1->y, 0.0L, 2 * trace1->label[i - 1]);
        fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", trace1->x[i - 1], trace1->y, delz, 2 * trace1->label[i - 1] + 1);

        WriteToPoint(&trace1->x[i - 1], &trace1->y, &(long double){0.0L}, &(int){2 * trace1->label[i - 1]}, pointoutput[3]);
        WriteToPoint(&trace1->x[i - 1], &trace1->y, &(long double){delz}, &(int){2 * trace1->label[i - 1] + 1}, pointoutput[3]);
  ...
  while (*(array->T + n) > a.p * a.c) {
    ...
    fprintf(pointoutput[0], "%.15Lf\t%.15Lf\n", foil->x[k], foil->y[k]);
    //   printf("t(%d) = %.15Lf\t%.15Lf\t%.15Lf\n", foil->label[k], foil->associatedT[k], foil->x[k], foil->y[k]);

    fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[k], foil->y[k], 0.0L, 2 * foil->label[k]);
    fprintf(pointoutput[1], "%.15Lf\t%.15Lf\t%.4Lf\t%d\n", foil->x[k], foil->y[k], delz, 2 * foil->label[k] + 1);

    WriteToPoint(&foil->x[k], &foil->y[k], &(long double){0.0L}, &(int){2 * foil->label[k]}, pointoutput[3]);
    WriteToPoint(&foil->x[k], &foil->y[k], &(long double){delz}, &(int){2 * foil->label[k] + 1}, pointoutput[3]);

    fprintf(pointoutput[2], "(%.15Lf %.15Lf %.4Lf )\n", foil->x[k], foil->y[k], 0.0L);
    fprintf(pointoutput[2], "(%.15Lf %.15Lf %.4Lf )\n", foil->x[k], foil->y[k], delz);
...
void Tvalues(int mp, long double r, long double t[4], Tarray *array, int *defaulter, int gp) {
  long double A, B, C;
  long double A1, B1;
...
void arcadjust(Tarray *array, aerofoil a, int *ncrit, int *defaulter) {
  ...
  long double y0 = f1(array->T[0], a) + f3(array->T[0], a) * cosl(theta1(array->T[0], a));
  long double y1 = f1(array->T[1], a) + f3(array->T[1], a) * cosl(theta1(array->T[1], a));
  long double x0 = array->T[0] - f3(array->T[0], a) * sin(theta1(array->T[0], a));
  long double x1 = array->T[1] - f3(array->T[1], a) * sin(theta1(array->T[1], a));
  long double d1, d2;
...
int main(int argc, char *argv[]) {
  ...
  default:
  badcase:
    printf("\n Too many arguments, enter the name of the input file as the only argument.\n");
    printf("File must follow the following format:\n\n");
    printf("xL = ...\n");
    printf("xR = ...\n");
    printf("yB = ...\n");
    printf("yU = ...\n");
    ...
    printf("cpar = ...\n");
    printf("tpar = ...\n");
    printf("\n\n Where xL is the leftmost coordinate of the mesh, xR is the rightmost coordiante of the mesh, yU is the uppermost coordinate of the mesh\n");
    printf("m is the camber, P is the position of the maximum camber, T is the thickness, c is the chord length\n");
    printf("mp is the number of points between the horizontal median point and the point which connects the top left point of the mesh\n mp x r_num/r_dem is the number of points between the horizontal median point and the point which connects the bottom left point of the mesh\n, gp is the number of points between the mp-th point and the tip of the airfoil\n, cpar is the coefficient which dicates spacing of points\n, and tpar is the coefficient which dictates the density of the tail.\n");
    return 4;
  };
  ...
  scanf("%Lf %Lf %Lf %Lf", &aero.m, &aero.p, &aero.T, &aero.c);
  ...
  justforfun:
    for (int i1 = -9; i1 <= 9; i1++) {
      testval = prevtval + fixdigit + ((long double)i1) / pow(10.0, (long double)(j + 3 + refiner));
      long double temp2val[19];
      temp2val[i1 + 9] = Zeroroot(testval, aero);
      if (i1 == -9) {
        deroundee = fixdigit + ((long double)i1) / pow(10.0, (long double)(j + 3 + refiner));
        ...
        if ((i1 == 9) && (fabsl(deroundee - fixdigit) <= 1e-17L) && (j + 3 + refiner < 16)) {
          refiner++;
          goto justforfun;
        }
      }
...
t1fix:
  ...
  justforfun1:
    for (int i1 = -9; i1 <= 9; i1++) {
      ...
      if ((i1 == 9) && (fabsl(deroundee - fixdigit) <= 1e-17L) && (refiner < 16)) {
        refiner++;
        goto justforfun1;
      }
    }
  ...
  if (fabsl(Firstroot(t1, aero, xL, yU)) > 0.01) {
    int ii = 0;
    printf("First %.15Lf !=0. (2 to increment exponent, 1 to decrement exponent, 0 to ignore", Secondroot(t2, aero, xL, yB));
    scanf("%d", &ii);
    if (ii == 1) {
      j--;
      goto t1fix;
...
t2fix:
  ...
  if (fabsl(Secondroot(t2, aero, xL, yB)) > 0.01) {
    int ii = 0;
    printf("Secondroot %.15Lf !=0. (2 to increment exponent, 1 to decrement exponent, 0 to ignore", Secondroot(t2, aero, xL, yB));
    scanf("%d", &ii);
    if (ii == 1) {
      j--;
      goto t2fix;
  ...
  justforfun3:
    for (int i1 = -9; i1 <= 9; i1++) {
      testval = prevtval + fixdigit + ((long double)i1) / pow(10.0, (long double)(j + 1 + refiner));

      long double temp2val[19];
      temp2val[i1 + 9] = Thirdroot(testval, aero);
      if (i1 == -9) {
        deroundee = fixdigit + ((long double)i1) / pow(10.0, (long double)(j + 1 + refiner));
        minval = temp2val[i1 + 9];
      ...
      if ((i1 == 9) && (fabsl(deroundee - fixdigit) <= 1e-17L) && (refiner < 16)) {
        printf("The digit was found to be 0. Heres what it is %.15Lf = %.15Lf\n", deroundee, fixdigit);
      }
      if ((i1 == 9) && (fabsl(deroundee - fixdigit) <= 1e-17L) && (refiner < 16)) {
        refiner++;
        goto justforfun3;
  ...
  fprintf(velocity, "}\n");
  fprintf(velocity, "// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //\n\n");
  fprintf(velocity, "dimensions      [0 1 -1 0 0 0 0];\n\n");
  fprintf(velocity, "\tinternalField   uniform (%.5Lf %.5Lf 0);\n\n", 24 * cosl(pi * angle / 180.0), 24 * sinl(pi * angle / 180.0));
  ...
  long double cpar = 0.0041 * (30.0 / mp);
  printf("ncrit before =%d\n", ncrit);
  OPTIONB opty = four;
  linestack(points, foil1, foilzoinks, aero, cpar, *array, ncrit, xL, mp, r_num, r_dem, xR, yU, yB, backtrace, trace1, slopeshift, faces, cells, owner, neighbour, boundary, opty);
