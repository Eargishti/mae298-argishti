...
void checkface(int ID, FILE *faces, FILE *points, FILE *output) {
  ...
  for (int i = 0; i <= ID; i++) {
    fscanf(faces, "%d %d %d %d %d %d", &p[0], &p[1], &p[2], &p[3], &bc,
           &flabel);
  };
  for (int k = 0; k < 4; k++) {
...
void calcdif(double *nodes, int nnodes){
...
void checkall(FILE *faces, FILE *points, FILE *output, int amount,
              int startFace) {
  faces = fopen("facecheck", "r");
  points = fopen("figarosafe.txt", "r");
  output = fopen("resultant.txt", "w");
